...
async def rm_deletedacc(show):
    ...
    if not admin and not creator:
        await show.edit("`admin nhj hu bsdk!`")
        return

    await show.edit("`KAM CHALU H \nAdmin to dedo?!?!`")
    del_u = 0
    del_a = 0
    ...
    if del_u > 0:
        del_status = f"Nikl zombie **{del_u}** deleted account(s)"

    if del_a > 0:
        del_status = f"Nikl bhutni **{del_u}** deleted account(s) \
    ...
    if Config.G_BAN_LOGGER_GROUP is not None:
        await show.client.send_message(
            Config.G_BAN_LOGGER_GROUP, "#PehlifursatseMargye\n"
            f"Nikl **{del_u}** deleted account(s) !!\
